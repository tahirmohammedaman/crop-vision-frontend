...
export function PredictionDetailPage() {
  ...
  React.useEffect(() => {
    if (item?.corrected_label) {
      setSelectedDisease(item.corrected_label)
    }
  ...
  const handleFeedback = React.useCallback(async (isCorrect: boolean) => {
    ...
    if (!isCorrect && !selectedDisease) {
      setFeedbackError('Select the correct disease before submitting.')
    ...
    try {
      ...
      try {
        const refreshed = await historyApi.getById(item.id)
        if (refreshed) nextItem = refreshed
      } catch {
        // ignore refresh errors and fall back to local state update
      }

      if (!nextItem) {
        nextItem = {
          ...item,
          confirmed: isCorrect ? true : false,
          corrected_label: isCorrect ? null : selectedDisease,
        }
      }
      ...
      const successMessage = isCorrect ? 'Prediction marked as correct.' : 'Correction submitted.'
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(0,1.25fr)]">
          <div className="space-y-5">
            ...
            <ReviewActions
              ...
              diseases={diseases}
              onMarkCorrect={() => handleFeedback(true)}
              onSubmitCorrection={() => handleFeedback(false)}
            />
